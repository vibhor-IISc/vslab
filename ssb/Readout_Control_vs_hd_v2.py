...
class Readout(UHFLI):
    ...
    def _uhf_initial_setup(self):
        ...
        self.demods[0].oscselect(0)
        self.demods[1].oscselect(1)
        self.demods[2].oscselect(2)
        self.demods[3].oscselect(3)
        self.demods[4].oscselect(4)
        self.demods[5].oscselect(5)
        self.demods[6].oscselect(6)
        # special command for SSB
        # to set 8th Demod to 4th Osc
        self.demods[7].oscselect(3)
        pass


    def modulation_phase_shift(self, phase):
        self.demods[7].phaseshift(phase)
        pass
...
##########  Potential issue in sideband_diff
    ...
    def optimize_dc(self, plot = False):
        ...
        file_path = r'E:\anshika\ssb_codes\Readout_IQ.txt'
        if isfile(file_path):
        ...
        # Finer sweep
        i_arr = np.linspace(opt_i-10.*1e-3, opt_i+10.*1e-3, 11)
        q_arr = np.linspace(opt_q-10.*1e-3, opt_q+10.*1e-3, 11)
        opt_i, opt_q = self._dc_sweep(i_arr, q_arr, plot)
    ...
    def optimize_phase(self, plot=False):
        ...
        # Writing to a file
        file_path = r'E:\anshika\ssb_codes\Readout_phase.txt'
        save = True
        if save:
    ...
    def phase_shift_relative(self, value):
        phase_3 = daq_uhf.getDouble(f"/{device_uhf}/demods/3/phaseshift")
        phase_7 = daq_uhf.getDouble(f"/{device_uhf}/demods/7/phaseshift")
        daq_uhf.setDouble(f"/{device_uhf}/demods/3/phaseshift", phase_3+value)
        daq_uhf.setDouble(f"/{device_uhf}/demods/7/phaseshift", phase_7+value)
        pass
...
# (daq_hdawg, device_hdawg, _) = create_api_session('dev8614', 6)
# api_server_version_check(daq_hdawg)

from zhinst.toolkit.driver.devices.hdawg import HDAWG
#from qcodes.instrument_drivers.stanford_research.SG396 import SRS_SG396
# from qcodes.instrument_drivers.stanford_research.SG396 import SRS_SG396
# from vslab.instrument_drivers.SG396 import SRS_SG396

# sg1 = SRS_SG396('sg1', SG2_ADDRESS) # Mixure LO for drive signal
# sg1.enable_RF('ON') 
# SGS_ADDRESS = 'TCPIP0::169.254.2.20::inst0::INSTR'
# sg1 = RohdeSchwarz_SGS100A('sg1', SGS_ADDRESS)
#sg1 =SRS_SG396('sg2', SG2_ADDRESS)
#sg1.status(True)
#sg1.setup_IQ_mod_ext()
# sg1.enable_RF("ON")

class Control(HDAWG):
    def __init__(self, address: str, host, ch0,ch1,
                 reset: bool = False, **kwargs: Any):
        super().__init__(address, host,**kwargs)
        ...
        daq_hdawg.setInt(f"/{device_hdawg}/sigouts/{self.ch0}/filter", 1)
        daq_hdawg.setInt(f"/{device_hdawg}/sigouts/{self.ch1}/filter", 1)
        daq_hdawg.setInt(f"/{device_hdawg}/sigouts/{self.ch0}/direct", 1)
        daq_hdawg.setInt(f"/{device_hdawg}/sigouts/{self.ch1}/direct", 1)
    ...
    def _hdawg_setup(self, setup):
        ...
        if setup == 1:
            # i_dc_ch.offset(0.0)
            daq_hdawg.setDouble(f'/{device_hdawg}/sigouts/{self.ch0}/offset', 0)
            # q_dc_ch.offset(0.0)
            daq_hdawg.setDouble(f'/{device_hdawg}/sigouts/{self.ch1}/offset', 0)
            # i_dc_ch.on(1)
            daq_hdawg.setInt(f'/{device_hdawg}/sigouts/{self.ch0}/on', 1)
            # q_dc_ch.on(1)
            daq_hdawg.setInt(f'/{device_hdawg}/sigouts/{self.ch1}/on', 1)
            # sin0.enables0(0)
            daq_hdawg.setInt(f'/{device_hdawg}/sines/{self.ch0}/enables/0', 0)
            # sin1.enables1(0)
            daq_hdawg.setInt(f'/{device_hdawg}/sines/{self.ch1}/enables/1', 0)
            pass
        elif setup == 2:
            # osc0.freq(IF)
            daq_hdawg.setDouble(f'/{device_hdawg}/oscs/0/freq', self.mix_freq)
            # aw0.gain1(awg_gain)
            daq_hdawg.setDouble(f'/{device_hdawg}/awgs/{self.awg0}/outputs/0/gains/0', self.awg_gain)
            # aw0.gain2(awg_gain)
            daq_hdawg.setDouble(f'/{device_hdawg}/awgs/{self.awg0}/outputs/1/gains/1', self.awg_gain)
            # sin0.amplitudes0(awg_sin_amp)
            daq_hdawg.setDouble(f'/{device_hdawg}/sines/{self.ch0}/amplitudes/0', self.awg_sin_amp)
            # sin1.amplitudes1(awg_sin_amp)
            daq_hdawg.setDouble(f'/{device_hdawg}/sines/{self.ch1}/amplitudes/1', self.awg_sin_amp)
            # sin0.enables0(1)
            daq_hdawg.setInt(f'/{device_hdawg}/sines/{self.ch0}/enables/0', 1)
            # sin1.enables1(1)
            daq_hdawg.setInt(f'/{device_hdawg}/sines/{self.ch1}/enables/1', 1)
            pass
        elif setup == 3:     
            # aw0.gain1(awg_gain)
            daq_hdawg.setDouble(f'/{device_hdawg}/awgs/{self.awg0}/outputs/0/gains/0', self.awg_gain)
    ...
    def _dc_sweep(self, i_arr, q_arr, plot = True):
        ...
        for index, i in enumerate(i_arr):
            # i_dc_ch.offset(i)
            daq_hdawg.setDouble(f'/{device_hdawg}/sigouts/{self.ch0}/offset', i)
            pow_arr.append([])
            progress_bar.update(index+1)
            for idq, q in enumerate(q_arr):
                # q_dc_ch.offset(q)
                daq_hdawg.setDouble(f'/{device_hdawg}/sigouts/{self.ch1}/offset', q)
                if idq == 0:
                    sleep(0.050)
                pow_arr[-1].append(self._get_power())
        progress_bar.finish()
        ...
        daq_hdawg.setDouble(f'/{device_hdawg}/sigouts/{self.ch0}/offset', i_arr[indices[0][0]])
        # q_dc_ch.offset(q_arr[indices[1][0]])
        daq_hdawg.setDouble(f'/{device_hdawg}/sigouts/{self.ch1}/offset', q_arr[indices[1][0]])
    ...
    def _optimize_ph(self, ph_arr, plot = True):
        ...
        for index, phase in enumerate(ph_arr):
            ...
            daq_hdawg.setDouble(f'/{device_hdawg}/sines/{self.ch1}/phaseshift', phase)
            sa.sweep_single()
            diffs.append(self._sideband_diff())
            progress_bar.update(index+1)
        progress_bar.finish()
        ...
        daq_hdawg.setDouble(f'/{device_hdawg}/sines/{self.ch1}/phaseshift', ph_opt)
        if plot:
            plt.plot(ph_arr, diffs, '-ro')
            plt.title('Phase calibration result')
            plt.xlabel('Phase difference (degree)')
    ...
    def optimize_phase(self, plot=True):
        ...
        if self.sideband == 'right':
            ph_arr = np.linspace(-110, -80, 21)
        elif self.sideband == 'left':
            ph_arr = np.linspace(80, 100, 21)
        else:
            raise Exception('Use sideband = left or right')
        # Coarse sweep
        opt_ph = self._optimize_ph(ph_arr, plot)
        # Fine sweep
        ph_arr = np.linspace(opt_ph - 1., opt_ph + 1., 21)
        opt_ph = self._optimize_ph(ph_arr, plot)
        # Finer sweep
        ph_arr = np.linspace(opt_ph - 0.1, opt_ph + 0.1, 21)
        sa.sweep_cont()
        ...
        daq_hdawg.setDouble(f'/{device_hdawg}/sines/{self.ch1}/phaseshift', opt_ph)
        return opt_ph
    ...
    def _optimize_amp(self, q_arr, plot=True):
        ...
        for index, q in enumerate(q_arr):
            # sin1.amplitudes1(q)
            daq_hdawg.setDouble(f'/{device_hdawg}/sines/{self.ch1}/amplitudes/1', q)
            diffs.append(self._sideband_diff())
            progress_bar.update(index+1)
        progress_bar.finish()
    ...
    def optimize_amplitude(self, plot=True):
        self._sa_setup(2)
        self._hdawg_setup(3)
        q_arr = np.linspace(self.awg_sin_amp-0.05, self.awg_sin_amp+0.05,31)
        opt_q = self._optimize_amp(q_arr, plot)
        # sin1.amplitudes1(opt_q)
        daq_hdawg.setDouble(f'/{device_hdawg}/sines/{self.ch1}/amplitudes/1', opt_q)
        
        # Fine sweep
        q_arr = np.linspace(opt_q-0.01, opt_q+0.01, 21)
        opt_q = self._optimize_amp(q_arr, plot)
        # sin1.amplitudes1(opt_q)
        daq_hdawg.setDouble(f'/{device_hdawg}/sines/{self.ch1}/amplitudes/1', opt_q)
        # Finer sweep

        q_arr = np.linspace(opt_q-0.003, opt_q+0.003, 11)
        opt_q = self._optimize_amp(q_arr, plot)
        # sin1.amplitudes1(opt_q)
        daq_hdawg.setDouble(f'/{device_hdawg}/sines/{self.ch1}/amplitudes/1', opt_q)
        sa.sweep_cont()
    ...
    def post_calibration_ready(self):
        ...
        # sin1.enables1(0)
        daq_hdawg.setInt(f'/{device_hdawg}/sines/{self.ch1}/enables/1', 0)
        daq_hdawg.setInt(f'/{device_hdawg}/awgs/{self.awg0}/outputs/0/modulation/mode', 1)
        daq_hdawg.setInt(f'/{device_hdawg}/awgs/{self.awg0}/outputs/1/modulation/mode', 1)
        # aw0.gain1(sin0.amplitudes0())
        sin0amp0 = daq_hdawg.getDouble(f'/{device_hdawg}/sines/{self.ch0}/amplitudes/0')
        daq_hdawg.setDouble(f'/{device_hdawg}/awgs/{self.awg0}/outputs/0/gains/0', sin0amp0)
        # aw0.gain2(sin1.amplitudes1())
        sin1amp1 = daq_hdawg.getDouble(f'/{device_hdawg}/sines/{self.ch1}/amplitudes/1')
        daq_hdawg.setDouble(f'/{device_hdawg}/awgs/{self.awg0}/outputs/1/gains/1', sin1amp1)
        pass

    def osc_amp(self, value):
        mod_amp_1 = daq_hdawg.getDouble(f"/{device_hdawg}/sines/{self.ch0}/amplitudes/0")
        mod_amp_2 = daq_uhf.getDouble(f"/{device_hdawg}/sines/{self.ch1}/amplitudes/1")
        ratio_iq = mod_amp_1 / mod_amp_2
        daq_hdawg.setDouble(f"/{device_hdawg}/sines/{self.ch0}/amplitudes/0", value*ratio_iq)
        daq_hdawg.setDouble(f"/{device_hdawg}/sines/{self.ch1}/amplitudes/1", value)
        pass
    ...
    def osc_amp_pulse(self, value):
        mod_amp_1 = daq_hdawg.getDouble(f"/{device_hdawg}/awgs/0/outputs/{self.ch0}/gains/0")
        mod_amp_2 = daq_uhf.getDouble(f"/{device_hdawg}/awgs/0/outputs/{self.ch1}/gains/1")
        ratio_iq = mod_amp_1 / mod_amp_2
        daq_hdawg.setDouble(f"/{device_hdawg}/awgs/{self.awg0}/outputs/{self.ch0}/gains/0", value*ratio_iq)
        daq_hdawg.setDouble(f"/{device_hdawg}/awgs/{self.awg0}/outputs/{self.ch1}/gains/1", value)
        pass
    ...
    def pulse_mode_on(self):
        self.post_calibration_ready()
        #daq_hdawg.setDouble(f"/{device_hdawg}/awgs/0/outputs/0/modulation/mode/0",OFF)
        #daq_hdawg.setDouble(f"/{device_hdawg}/awgs/0/outputs/1/modulation/mode/0")
