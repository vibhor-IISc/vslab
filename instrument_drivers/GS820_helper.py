...
def GS820sweepVoltTo(gs820, chn=1,  setV=0, incr = 1e-3, tol=2e-3):
    sign = np.sign(setV -float(gs820.ask(f'CHAN{chn}:SOUR:LEV?')))
    dummy = gs820.ask('*OPC?')
    while np.abs(float(gs820.ask(f'CHAN{chn}:SOUR:LEV?')) - setV) >= tol:
        dummy = gs820.ask('*OPC?')
        now = float(gs820.ask(f'CHAN{chn}:SOUR:LEV?'))
        dummy = gs820.ask('*OPC?')
        gs820.write(f'CHAN{chn}:SOUR:LEV {now + sign*incr}')
    if np.abs(float(gs820.ask(f'CHAN{chn}:SOUR:LEV?')) - setV) < tol:
        dummy = gs820.ask('*OPC?')
