...
class psd(ParameterWithSetpoints):
    def get_raw(self):
        ...
        return np.array(_tr.split(',')).astype(float)
...
class ZVL_SA(VisaInstrument):
    ...
    def __init__(self, name       : str,
        ...
        self.add_parameter(name       = 'bandwidth',
                           ...
                           vals       = vals.Enum(*np.append(10**7, np.kron([1, 3], 10 ** np.arange(1,7))))
                           )
        ...
        self.add_parameter(name       = 'reference_level',
                           ...
                           vals = vals.Ints(min_value = -130, max_value = 20)
                           )
    ...
    def markers_add_at(self, mlist):
        if len(mlist)<=4:
            ...
            raise Exception("Sorry, only 4 markers are allowed on this model") 


    def _markers_X(self):
      _ls = []
      for idx, m in enumerate(np.arange(4)):
