...
row_cut_source = ColumnDataSource(data={"x": xcoord, "y": []})
col_cut_source = ColumnDataSource(data={"x": ycoord, "y": []})
...
def update_linecuts(attr, old, new):
    ...
    row_cut_source.data = {"x": np.arange(initial_array.shape[1]), "y": initial_array[row_index, :]}
    col_cut_source.data = {"x": np.arange(initial_array.shape[0]), "y": initial_array[:, col_index]}
